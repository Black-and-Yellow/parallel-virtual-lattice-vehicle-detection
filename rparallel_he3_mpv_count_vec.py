...
# Optimized grid processing with smart contour detection
def process_grid_optimized(roi_data, grid_w, grid_h, num_rows, num_cols):
    """
    Optimized grid processing with accurate contour detection
    Uses pre-filtering to reduce contour calls while maintaining accuracy
    """
    ...
    # Smart grid evaluation with contours
    PIXEL_PREFILTER = 50  # Skip contours if < 50 white pixels
    CONTOUR_THRESHOLD = 100  # Minimum contour area
    ...
    for row in range(num_rows):
        ...
        for col in range(num_cols):
            ...
            # Stage 1: Fast pixel pre-filter (vectorized)
            white_count = np.count_nonzero(cell)
            
            if white_count < PIXEL_PREFILTER:
                # Definitely empty - skip contour detection
                continue
            
            # Stage 2: Accurate contour detection for candidates
            ...
            if any(cv2.contourArea(c) >= CONTOUR_THRESHOLD for c in contours):
                result[row, col] = 1
